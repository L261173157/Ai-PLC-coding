...
(Claude Code、Cursor)能驱动 TIA 工程:连接、列出/读/导入/导出 块与 tag、编译、下载、运行/停车。
这里没有 LLM/generator/RAG 层——就是 MCP server。工具面与阶段状态在 `README.md`;本文件覆盖架构,
以及一次改动通常触及的非显然机制。

**架构(2026-07-23 重构为多品牌 monorepo):** 仓库按品牌 + 通用层组织。西门子 TIA 品牌单元在 `brands/tia/`:能力层 = MCP server,在 `brands/tia/mcp/`(`brands/tia/mcp/src/` + `brands/tia/mcp/TiaMcp.slnx` + `brands/tia/mcp/docs/`);品牌知识层 = `brands/tia/skills/`(渐进式 `SKILL.md`,英文命令名 + 中文正文)。品牌中性的通用方法论在仓库根 `skills/`(目前为桩);公司标准**不进本仓库**——`standards/` 只是挂载点,内容来自独立上层私有库(见 `standards/README.md`)。Claude Code 插件打包在 `plugins/`:`plugins/plc-siemens/`(TIA mcp + 品牌 skills,`.claude-plugin/plugin.json` + `.mcp.json` 用 `${CLAUDE_PLUGIN_ROOT}/../../brands/tia/...` 指源)+ `plugins/plc-base/`(通用 skills,无 mcp)。编排层 = agent。详见 `docs/architecture.md`。
...
⚠️ **改了 worker 必须单独重编它,改动才会生效。** `dotnet build TiaMcp.slnx` **不含** net48 worker
(`OpennessEngine.cs` / `Program.cs`)——它故意不进解决方案。所以凡改后端逻辑(设备/块/标签/网络的读
写、设备枚举等)都要**额外**跑一次 `dotnet build brands/tia/mcp/src/TiaMcp.Openness.Worker/...csproj`,
否则 server 运行期仍加载旧 worker,新代码完全不生效(而 `--selftest` 走 Fake 也发现不了)。曾因此让
"设备组递归"(ListTargets/ResolveDevice 改用 EnumerateAllDevices)一度没起作用,排查了很久。

`brands/tia/mcp/tests/` 下另有 `read_full_project.py`(整项目只读通读,落 JSON)、`write_demo_project.py`
(克隆现有块为 FB_DemoWrite 的写入演示)、`verify_fixes.py` / `verify_live_fixes.py`(离线/真机回归)——
它们是烟测之外的实弹脚本,改后端后建议跑一遍 `verify_live_fixes.py` 兜底。
